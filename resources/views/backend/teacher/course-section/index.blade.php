...
@section('content')
    <div class="page-content">
        ...
        <!--breadcrumb-->
        @include('backend.section.breadcrumb', ['title' => 'Course', 'sub_title' => 'Course Section'])
        <!--end breadcrumb-->

        <div class="row">
            <div class="col-12 col-lg-12">
                <div style="display: flex; align-items:center; justify-content:space-between">
                    <h6 class="mb-0 text-uppercase">All Content Section</h6>
                    <a href="{{ route('teacher.course.index') }}" class="btn btn-success mb-2">Back</a>
                </div>
                <hr />
                ...
                @foreach ($course_wise_lecture as $item)
                    <div class="card col-md-12 radius-10">
                        <div class="card-body">
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center">
                                    ...
                                    <svg style="cursor: pointer" data-bs-toggle="collapse"
                                        data-bs-target="#demo{{ $item->id }}" xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <!-- Title -->
                                    <div class="ms-3">
                                        <h6 class="mt-0 mb-0">{{ $item->section_title }}</h6>
                                    </div>
                            ...
                            <div class="mt-3 collapse show" id="demo{{ $item->id }}">
                                @foreach ($item['lecture'] as $lecture)
                                    ...
                                    <!-- Edit lecture Modal -->
                                    @include('backend.teacher.course-section.modal.lecture-edit-modal')
                                @endforeach
                            </div>
                    ...
                    {{-- Add Course Modal --}}
                    @include('backend.teacher.course-section.modal.lecture-create-modal')
                @endforeach
            </div>
        </div>
        ...
        @include('backend.teacher.course-section.modal.create-section-modal')
...
@push('scripts')
    ...
    <script>
        $(document).on('click', '.delete-section', function(e) {
            ...
            let Id = $(this).data('id');
            let deleteUrl = "{{ route('teacher.course-section.destroy', ':id') }}".replace(':id', Id);
    ...
    <script>
        $(document).on('click', '.delete-lecture', function(e) {
            ...
            let Id = $(this).data('id');
            let deleteUrl = "{{ route('teacher.lecture.destroy', ':id') }}".replace(':id', Id);
